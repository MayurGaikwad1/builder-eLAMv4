      ...
      <div class="flex items-center justify-between">
        ...
        <div class="flex items-center space-x-4">
          ...
          <div class="relative">
            ...
            >
              <!-- User Avatar -->
              <div *ngIf="currentUser()?.avatar" class="w-8 h-8 rounded-full overflow-hidden">
                <img
              ...
              <div *ngIf="!currentUser()?.avatar" class="w-8 h-8 bg-primary-100 rounded-full flex items-center justify-center">
                <span class="text-sm font-medium text-primary-700">{{ getUserInitials() }}</span>
              </div>

              <!-- User Info (hidden on mobile) -->
              <div class="hidden md:block text-left">
                <div class="text-sm font-medium text-secondary-900">{{ currentUser()?.name || 'User' }}</div>
                <div class="text-xs text-secondary-500">{{ getUserRole() }}</div>
              </div>
            ...
            >
              <div class="p-4 border-b border-secondary-200">
                <div class="flex items-center space-x-3">
                  <div *ngIf="currentUser()?.avatar" class="w-10 h-10 rounded-full overflow-hidden">
                    <img
                      [src]="currentUser()!.avatar"
                      [alt]="currentUser()!.name"
                      class="w-full h-full object-cover"
                  ...
                  <div *ngIf="!currentUser()?.avatar" class="w-10 h-10 bg-primary-100 rounded-full flex items-center justify-center">
                    <span class="text-sm font-medium text-primary-700">{{ getUserInitials() }}</span>
                  </div>
                  <div>
                    <div class="text-sm font-medium text-secondary-900">{{ currentUser()?.name || 'User' }}</div>
                    <div class="text-xs text-secondary-500">{{ currentUser()?.email || '' }}</div>
                    <div class="text-xs text-primary-600">{{ getUserRole() }}</div>
                  </div>
                </div>
              </div>
              ...
              <div class="py-2">
                ...
                >
                  <svg class="w-4 h-4 mr-3 text-secondary-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
