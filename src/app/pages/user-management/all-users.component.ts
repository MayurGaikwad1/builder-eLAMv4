...
import { UserProfile, SecurityClearance } from '../../shared/interfaces/user-management.interface';
    ...
    <div class="space-y-6">
      ...
      <div class="flex items-center justify-between">
        <div>
          ...
          <p class="text-secondary-600">Comprehensive user directory and management</p>
        </div>
      ...
      <div class="card">
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div>
            <label class="block text-sm font-medium text-secondary-700 mb-1">Search Users</label>
            <input 
              type="text"
              [(ngModel)]="searchQuery"
              (input)="onSearch()"
      ...
      <div class="card">
        <div class="flex items-center justify-between mb-6">
          <div>
            <h2 class="text-lg font-semibold text-secondary-900">Users ({{ filteredUsers().length }})</h2>
            <p class="text-sm text-secondary-600">{{ getActiveUsersCount() }} active • {{ getInactiveUsersCount() }} inactive</p>
          </div>
        ...
        <!-- Bulk Actions -->
        <div *ngIf="showBulkActions" class="mb-4 p-4 bg-primary-50 border border-primary-200 rounded-lg">
          <div class="flex items-center justify-between">
            <div class="flex items-center space-x-4">
              <span class="text-sm font-medium text-primary-900">{{ selectedUsers.size }} users selected</span>
              <button class="text-sm text-primary-600 hover:text-primary-700">Select All</button>
              <button class="text-sm text-primary-600 hover:text-primary-700">Clear Selection</button>
            </div>
            <div class="flex items-center space-x-2">
              <button class="btn-secondary text-sm py-1 px-3">Export Selected</button>
        ...
        <div class="overflow-x-auto">
          <table class="min-w-full divide-y divide-secondary-200">
            ...
            <tbody class="bg-white divide-y divide-secondary-200">
              <tr *ngFor="let user of filteredUsers(); trackBy: trackByUserId" class="hover:bg-secondary-50">
                ...
                <td class="table-cell">
                  <div class="flex items-center space-x-3">
                    ...
                    <div>
                      ...
                      <p class="text-xs text-secondary-500">{{ user.employeeId }}</p>
                    </div>
                  </div>
                </td>
                <td class="table-cell">
                  <div>
                    ...
                    <p class="text-xs text-secondary-500">{{ user.location }}</p>
                  </div>
                </td>
                <td class="table-cell">
                  <div class="flex items-center space-x-2">
                    ...
                    <svg *ngIf="user.mfaEnabled" class="w-4 h-4 text-success-600" fill="currentColor" viewBox="0 0 20 20" title="MFA Enabled">
                ...
                <td class="table-cell">
                  <div class="flex items-center space-x-2">
                    <span [class]="getRiskScoreClass(user.riskScore)" class="px-2 py-1 text-xs font-medium rounded-full">
                      {{ user.riskScore }}
                    </span>
                    <span [class]="getClearanceClass(user.securityClearance)" class="px-2 py-1 text-xs font-medium rounded-full">
                ...
                <td class="table-cell">
                  <div>
                    <p class="text-sm text-secondary-900">{{ user.lastActivity | date:'MMM d, y' }}</p>
                    <p class="text-xs text-secondary-500">{{ user.sessionCount }} sessions</p>
                  </div>
                </td>
                <td class="table-cell">
                  <div class="flex items-center space-x-2">
                    <span [class]="getComplianceClass(user.complianceStatus.isCompliant)" class="px-2 py-1 text-xs font-medium rounded-full">
                      {{ user.complianceStatus.isCompliant ? 'Compliant' : 'Non-Compliant' }}
                    </span>
                    <span *ngIf="user.complianceStatus.violations.length > 0" class="text-xs text-danger-600">
                ...
                <td class="table-cell">
                  <div class="flex items-center space-x-2">
                    ...
                    <button class="text-secondary-600 hover:text-secondary-700 text-sm">Edit</button>
                    <button 
...
export class AllUsersComponent implements OnInit {
  ...
  onSearch() {
    if (this.searchQuery.trim()) {
      this.userManagementService.searchUsers(this.searchQuery).subscribe(users => {
        this.filteredUsers.set(users);
      });
    } else {
      this.applyFilters();
  ...
  applyFilters() {
    ...
    if (this.filters.riskLevel) {
      ...
      const [min, max] = riskRanges[this.filters.riskLevel as keyof typeof riskRanges];
  ...
  deactivateUser(userId: string) {
    if (confirm('Are you sure you want to deactivate this user?')) {
      this.userManagementService.deactivateUser(userId, 'Manual deactivation').subscribe(() => {
        this.loadUsers();
      });
    }
  }
  ...
  getActiveUsersCount(): number {
    return this.filteredUsers().filter(u => u.status === UserStatus.Active).length;
  }
  ...
  getClearanceClass(clearance?: SecurityClearance): string {
    ...
    return clearance ? classes[clearance] : 'bg-secondary-100 text-secondary-800';
  }
  ...
  getComplianceClass(isCompliant: boolean): string {
    return isCompliant ? 'bg-success-100 text-success-800' : 'bg-danger-100 text-danger-800';
  }
