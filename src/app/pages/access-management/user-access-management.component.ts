  ...
  template: `
    <div class="space-y-6">
      ...
      <div class="flex items-center justify-between">
        <div>
          <h1 class="text-2xl font-bold text-secondary-900">User Access Management</h1>
          <p class="text-secondary-600">
            Manage user access through structured workflows with multi-level approvals
          </p>
        </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-5 gap-4">
        <div class="metric-card">
          <div class="text-center">
            <p class="text-2xl font-bold text-secondary-900">{{ totalRequests() }}</p>
            <p class="text-sm text-secondary-600">Total Requests</p>
          </div>
        </div>
        <div class="metric-card">
          <div class="text-center">
            <p class="text-2xl font-bold text-warning-600">{{ pendingRequests() }}</p>
            <p class="text-sm text-secondary-600">Pending Approval</p>
          </div>
        </div>
        <div class="metric-card">
          <div class="text-center">
            <p class="text-2xl font-bold text-success-600">{{ approvedRequests() }}</p>
            <p class="text-sm text-secondary-600">Approved</p>
          </div>
        </div>
        <div class="metric-card">
          <div class="text-center">
            <p class="text-2xl font-bold text-danger-600">{{ overdueRequests() }}</p>
            <p class="text-sm text-secondary-600">Overdue</p>
          </div>
        </div>
        <div class="metric-card">
          <div class="text-center">
            <p class="text-2xl font-bold text-primary-600">{{ autoProcessedToday() }}</p>
      ...
      <div *ngIf="showBulkUpload" class="card animate-slide-up">
        <div class="flex items-center justify-between mb-6">
          <h2 class="text-lg font-semibold text-secondary-900">Bulk Upload Access Requests</h2>
        ...
        <form (ngSubmit)="processBulkUpload()" class="space-y-4">
          <div>
            <label class="block text-sm font-medium text-secondary-700 mb-2">Select Application</label>
            <select
          ...
          <div>
            <label class="block text-sm font-medium text-secondary-700 mb-2">Upload Excel File</label>
            <div class="border-2 border-dashed border-secondary-300 rounded-lg p-6 text-center hover:border-primary-400 transition-colors">
              ...
              <div *ngIf="!selectedFile" (click)="fileInput.click()" class="cursor-pointer">
                ...
                <p class="text-secondary-600">Click to upload or drag and drop</p>
                <p class="text-xs text-secondary-500">Excel files only (.xlsx, .xls, .csv)</p>
              </div>
              <div *ngIf="selectedFile" class="space-y-2">
                <div class="flex items-center justify-center space-x-2">
                  ...
                  <span class="text-secondary-900">{{ selectedFile.name }}</span>
                </div>
                <button type="button" (click)="clearFile(); fileInput.click()" class="text-sm text-primary-600 hover:text-primary-700">
                  Change file
                </button>
              </div>
            </div>
          ...
          <div class="flex items-center justify-end space-x-3">
            <button type="button" (click)="cancelBulkUpload()" class="btn-secondary">Cancel</button>
            <button type="submit" [disabled]="!isUploadFormValid()" class="btn-primary">
              Process Upload
            </button>
          </div>
        </form>
        ...
        <!-- Upload History -->
        <div *ngIf="recentUploads().length > 0" class="mt-6 pt-6 border-t border-secondary-200">
          <h3 class="text-md font-medium text-secondary-900 mb-4">Recent Uploads</h3>
          <div class="space-y-3">
            <div *ngFor="let upload of recentUploads()" class="flex items-center justify-between p-3 bg-secondary-50 rounded-lg">
              <div class="flex items-center space-x-3">
                ...
                <div>
                  <p class="font-medium text-secondary-900">{{ upload.fileName }}</p>
                  <p class="text-sm text-secondary-600">
                    {{ upload.successfulRecords }}/{{ upload.totalRecords }} successful
                    • {{ upload.uploadedAt | date:'MMM d, y h:mm a' }}
                  </p>
                </div>
              </div>
              <div class="flex items-center space-x-2">
                <span [class]="getUploadStatusClass(upload.status)" class="px-2 py-1 text-xs font-medium rounded-full">
                  {{ getUploadStatusLabel(upload.status) }}
                </span>
                <button *ngIf="upload.downloadUrl" class="text-primary-600 hover:text-primary-700 text-sm">
                  Download Results
                </button>
              </div>
            </div>
      ...
      <div *ngIf="showNewRequest" class="card animate-slide-up">
        <h2 class="text-lg font-semibold text-secondary-900 mb-6">New Access Request</h2>

        <form (ngSubmit)="submitRequest()" class="space-y-6">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <label class="block text-sm font-medium text-secondary-700 mb-2">Application</label>
              <select
            ...
            <div>
              <label class="block text-sm font-medium text-secondary-700 mb-2">Access Level</label>
              <select
          ...
          <div>
            <label class="block text-sm font-medium text-secondary-700 mb-2">User IDs</label>
            <textarea
          ...
          <div>
            <label class="block text-sm font-medium text-secondary-700 mb-2">Business Justification</label>
            <textarea
          ...
          <!-- Workflow Preview -->
          <div *ngIf="selectedApplication()" class="p-4 bg-primary-50 border border-primary-200 rounded-lg">
            <h4 class="text-sm font-medium text-primary-900 mb-3">Approval Workflow</h4>
            <div class="flex items-center space-x-2">
              <div *ngFor="let level of selectedApplication()?.approvalWorkflow.approvalLevels; let i = index" 
                   class="flex items-center">
                <div class="flex items-center space-x-2">
                  <div class="w-8 h-8 bg-primary-100 text-primary-800 rounded-full flex items-center justify-center text-sm font-medium">
                    {{ level.level }}
                  </div>
                  <span class="text-sm text-primary-700">{{ level.role | titlecase }}</span>
                </div>
                <svg *ngIf="i < selectedApplication()!.approvalWorkflow.approvalLevels.length - 1" 
                     class="w-4 h-4 text-primary-400 mx-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path>
                </svg>
              </div>
            </div>
            <p class="text-xs text-primary-600 mt-2">
              Expected processing time: {{ selectedApplication()?.approvalWorkflow.deadlineHours }} hours
            </p>
          </div>

          <div class="flex items-center justify-end space-x-3">
            <button type="button" (click)="cancelNewRequest()" class="btn-secondary">Cancel</button>
            <button type="submit" [disabled]="!isRequestFormValid()" class="btn-primary">
              Submit Request
            </button>
          </div>
        </form>
      ...
      <div class="card">
        <div class="flex items-center justify-between mb-6">
          <h2 class="text-lg font-semibold text-secondary-900">Access Requests</h2>
          <div class="flex items-center space-x-3">
            <select 
        ...
        <div class="overflow-x-auto">
          <table class="min-w-full divide-y divide-secondary-200">
            ...
            <tbody class="bg-white divide-y divide-secondary-200">
              <tr *ngFor="let request of filteredRequests(); trackBy: trackByRequestId" class="hover:bg-secondary-50">
                <td class="table-cell">
                ...
                <td class="table-cell">
                  <div>
                    ...
                    <p class="text-xs text-secondary-500">{{ request.accessLevel | titlecase }}</p>
                  </div>
                </td>
                <td class="table-cell">
                  <div>
                    <p class="font-medium">{{ request.userIds.length }} users</p>
                    <p class="text-xs text-secondary-500">{{ request.userIds.slice(0, 2).join(', ') }}{{ request.userIds.length > 2 ? '...' : '' }}</p>
                  </div>
                </td>
                <td class="table-cell">
                ...
                <td class="table-cell">
                  <div *ngIf="request.status === 'in_review' || request.status === 'awaiting_approval'">
                    <p class="text-sm">Level {{ request.currentApprovalLevel }}</p>
                    <p class="text-xs text-secondary-500">{{ getCurrentApproverRole(request) | titlecase }}</p>
                  </div>
                  <span *ngIf="request.status === 'approved'" class="text-success-600 text-sm">Completed</span>
                  <span *ngIf="request.status === 'rejected'" class="text-danger-600 text-sm">Rejected</span>
                </td>
                <td class="table-cell">
                  <span [class]="getPriorityClass(request.priority)" class="px-2 py-1 text-xs font-medium rounded-full">
                    {{ request.priority | titlecase }}
                  </span>
                </td>
                <td class="table-cell">
                  <span [class]="getDeadlineClass(request.deadline)" class="text-sm">
                ...
                <td class="table-cell">
                  <div class="flex items-center space-x-2">
                    ...
                    <button *ngIf="canApprove(request)" class="text-success-600 hover:text-success-700 text-sm">
                      Approve
                    </button>
                  </div>
                </td>
...
export class UserAccessManagementComponent implements OnInit {
  ...
  downloadTemplate() {
    ...
    const template = "User ID,Application,Access Level,Justification,Department\n";
  ...
  processBulkUpload() {
    if (this.selectedFile && this.bulkUploadForm.applicationId) {
      this.accessManagementService.processBulkUpload(this.selectedFile, this.bulkUploadForm.applicationId)
        .subscribe(upload => {
  ...
  submitRequest() {
    if (this.isRequestFormValid()) {
      ...
      this.accessManagementService.createAccessRequest(request).subscribe(newRequest => {
  ...
  getCurrentApproverRole(request: UserAccessRequest): string {
    ...
    const level = app?.approvalWorkflow.approvalLevels.find(l => l.level === request.currentApprovalLevel);
    return level?.role || "";
  }
