...
import { AccessRequest, RequestType, UrgencyLevel, Role, RiskLevel } from '../shared/interfaces/user.interface';
    ...
    <div class="space-y-6">
      ...
      <div class="flex items-center justify-between">
        <div>
          ...
          <p class="text-secondary-600">Request and manage access to systems and resources</p>
        </div>
        <button 
      ...
      <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div class="metric-card">
          <div class="text-center">
            <p class="text-2xl font-bold text-secondary-900">{{ myRequests().length }}</p>
        ...
        <div class="metric-card">
          <div class="text-center">
            <p class="text-2xl font-bold text-success-600">{{ approvedCount() }}</p>
            <p class="text-sm text-secondary-600">Approved</p>
          </div>
        </div>
        <div class="metric-card">
          <div class="text-center">
            <p class="text-2xl font-bold text-primary-600">{{ avgProcessingTime() }}</p>
      ...
      <div *ngIf="showNewRequestForm" class="card animate-slide-up">
        ...
        <form (ngSubmit)="submitRequest()" class="space-y-6">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <label class="block text-sm font-medium text-secondary-700 mb-2">Request Type</label>
              <select 
            ...
            <div>
              <label class="block text-sm font-medium text-secondary-700 mb-2">Urgency Level</label>
              <select 
          ...
          <div>
            <label class="block text-sm font-medium text-secondary-700 mb-2">Requested Role/System</label>
            <select 
            ...
            >
              ...
              <option *ngFor="let role of availableRoles" [value]="role.id">{{ role.name }} - {{ role.description }}</option>
            </select>
          </div>

          <div *ngIf="newRequest.requestedRoles.length > 0">
            <label class="block text-sm font-medium text-secondary-700 mb-2">Selected Roles</label>
            <div class="space-y-2">
              <div *ngFor="let role of newRequest.requestedRoles" 
                   class="flex items-center justify-between p-3 bg-secondary-50 rounded-lg">
                <div class="flex items-center space-x-3">
                  <span [class]="getRiskBadgeClass(role.riskLevel)" class="px-2 py-1 text-xs font-medium rounded-full">
                    {{ role.riskLevel | titlecase }}
                  </span>
                  <div>
                    <p class="font-medium text-secondary-900">{{ role.name }}</p>
                    <p class="text-sm text-secondary-600">{{ role.description }}</p>
                  </div>
                </div>
                <button 
                  type="button"
          ...
          <div *ngIf="newRequest.requestedRoles.length > 0" class="p-4 bg-warning-50 border border-warning-200 rounded-lg">
            <div class="flex items-center mb-2">
              <svg class="w-5 h-5 text-warning-600 mr-2" fill="currentColor" viewBox="0 0 20 20">
                <path fill-rule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clip-rule="evenodd"></path>
              </svg>
              <p class="text-sm font-medium text-warning-800">Risk Assessment</p>
            </div>
            <p class="text-sm text-warning-700">
              This request will require additional approval due to {{ getHighestRiskLevel() }} risk level.
              Expected processing time: {{ getExpectedProcessingTime() }}
            </p>
          </div>
      ...
      <div class="card">
        <div class="flex items-center justify-between mb-6">
          ...
          <div class="flex items-center space-x-3">
            ...
            >
            <select class="border border-secondary-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary-500">
        ...
        <div class="overflow-x-auto">
          <table class="min-w-full divide-y divide-secondary-200">
            ...
            <tbody class="bg-white divide-y divide-secondary-200">
              <tr *ngFor="let request of myRequests(); trackBy: trackByRequestId" class="hover:bg-secondary-50">
                <td class="table-cell">
                  <span class="font-mono text-sm">{{ request.id }}</span>
                </td>
                <td class="table-cell">
                  <span class="capitalize">{{ request.requestType.replace('_', ' ') }}</span>
                </td>
                <td class="table-cell">
                  <div>
                    <p class="font-medium">{{ request.requestedRoles[0]?.name || 'Multiple Roles' }}</p>
                    <p class="text-xs text-secondary-500">{{ request.requestedResources.join(', ') }}</p>
                  </div>
                </td>
                <td class="table-cell">
                ...
                <td class="table-cell">
                  <span class="text-sm">{{ request.submittedAt | date:'MMM d, y' }}</span>
                </td>
                <td class="table-cell">
                  <div class="flex items-center space-x-2">
                    <button class="text-primary-600 hover:text-primary-700 text-sm">View</button>
                    <button *ngIf="request.status === 'draft'" class="text-secondary-600 hover:text-secondary-700 text-sm">Edit</button>
                  </div>
                </td>
              </tr>
            </tbody>
...
export class AccessRequestsComponent implements OnInit {
  ...
  availableRoles: Role[] = [
    ...
    { id: 'r4', name: 'System Administrator', description: 'Full system administration', permissions: [], riskLevel: RiskLevel.Critical },
    { id: 'r5', name: 'Auditor', description: 'Audit and compliance access', permissions: [], riskLevel: RiskLevel.Medium },
    { id: 'r6', name: 'Developer', description: 'Development environment access', permissions: [], riskLevel: RiskLevel.Low },
    { id: 'r7', name: 'Manager', description: 'Management and approval access', permissions: [], riskLevel: RiskLevel.Medium }
  ];
  ...
  removeRole(roleId: string) {
    this.newRequest.requestedRoles = this.newRequest.requestedRoles.filter(r => r.id !== roleId);
    this.newRequest.requestedResources = this.newRequest.requestedResources.filter((_, index) => 
      this.newRequest.requestedRoles[index]?.id !== roleId
    );
  }

  submitRequest() {
    if (this.isFormValid()) {
      this.mockDataService.submitAccessRequest({
  ...
  isFormValid(): boolean {
    return this.newRequest.requestType !== '' && 
  ...
  pendingCount() {
    return this.myRequests().filter(r => r.status === 'submitted' || r.status === 'in_review').length;
  }

  approvedCount() {
    return this.myRequests().filter(r => r.status === 'approved' || r.status === 'completed').length;
  }
