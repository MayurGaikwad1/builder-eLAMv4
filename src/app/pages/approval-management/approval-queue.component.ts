    ...
    <div class="space-y-6">
      ...
      <div class="flex items-center justify-between">
        <div>
          ...
          <p class="text-secondary-600">Review and process pending access requests</p>
        </div>
      ...
      <div class="card">
        <div class="grid grid-cols-1 md:grid-cols-5 gap-4">
          <div>
            <label class="block text-sm font-medium text-secondary-700 mb-1">Search</label>
            <input 
              type="text"
              [(ngModel)]="searchQuery"
              (input)="applyFilters()"
          ...
          <div>
            <label class="block text-sm font-medium text-secondary-700 mb-1">Urgency</label>
            <select 
          ...
          <div>
            <label class="block text-sm font-medium text-secondary-700 mb-1">Request Type</label>
            <select 
          ...
          <div>
            <label class="block text-sm font-medium text-secondary-700 mb-1">Queue</label>
            <select 
      ...
      <!-- Bulk Actions Bar -->
      <div *ngIf="bulkMode && selectedRequests.size > 0" class="card bg-primary-50 border-primary-200">
        <div class="flex items-center justify-between">
          <div class="flex items-center space-x-4">
            <span class="text-sm font-medium text-primary-900">{{ selectedRequests.size }} requests selected</span>
            <button 
              (click)="selectAll()"
      ...
      <div class="card">
        <div class="flex items-center justify-between mb-6">
          <div>
            <h2 class="text-lg font-semibold text-secondary-900">Pending Requests ({{ filteredRequests().length }})</h2>
            <p class="text-sm text-secondary-600">
              {{ getHighPriorityCount() }} high priority • {{ getSlaBreachCount() }} SLA breaches
            </p>
          </div>
          <div class="flex items-center space-x-2">
            <button 
              (click)="sortBy('submittedAt')"
              [class]="sortField === 'submittedAt' ? 'btn-primary' : 'btn-secondary'"
            ...
            <button 
              ...
              [class]="sortField === 'urgency' ? 'btn-primary' : 'btn-secondary'"
              class="text-sm py-1 px-3"
            >
              Priority
            </button>
            <button 
              ...
              [class]="sortField === 'riskScore' ? 'btn-primary' : 'btn-secondary'"
              class="text-sm py-1 px-3"
            >
              Risk
            </button>
          ...
          <table class="min-w-full divide-y divide-secondary-200">
            ...
            <tbody class="bg-white divide-y divide-secondary-200">
              <tr *ngFor="let request of filteredRequests(); trackBy: trackByRequestId" 
                ...
                <td class="table-cell">
                  <div>
                    <p class="font-medium text-secondary-900">{{ request.requestTitle }}</p>
                    <p class="text-sm text-secondary-600">{{ request.description | slice:0:80 }}{{ request.description.length > 80 ? '...' : '' }}</p>
                ...
                <td class="table-cell">
                  <div class="flex items-center space-x-3">
                    ...
                    <div>
                      <p class="font-medium text-secondary-900">{{ request.requestedBy.name }}</p>
                      <p class="text-sm text-secondary-600">{{ request.requestedBy.department }}</p>
                      <p class="text-xs text-secondary-500">{{ request.requestedBy.title }}</p>
                    </div>
                  </div>
                </td>
                
                <td class="table-cell">
                  <span [class]="getRequestTypeClass(request.requestType)" class="px-2 py-1 text-xs font-medium rounded-full">
                ...
                <td class="table-cell">
                  <div class="flex items-center space-x-2">
                    <span [class]="getUrgencyClass(request.urgency)" class="px-2 py-1 text-xs font-medium rounded-full">
                      {{ request.urgency | titlecase }}
                    </span>
                    <svg *ngIf="request.slaBreachWarning" class="w-4 h-4 text-danger-500" fill="currentColor" viewBox="0 0 20 20">
                ...
                <td class="table-cell">
                  <div class="flex items-center space-x-2">
                    <span [class]="getRiskScoreClass(request.riskScore)" class="px-2 py-1 text-xs font-medium rounded-full">
                      {{ request.riskScore }}
                    </span>
                    <svg *ngIf="request.conflictChecks.length > 0" class="w-4 h-4 text-warning-500" fill="currentColor" viewBox="0 0 20 20" title="SoD Conflicts Detected">
                ...
                <td class="table-cell">
                  <div>
                    <span [class]="getSlaStatusClass(request)" class="text-sm font-medium">
        ...
        <div class="flex items-center justify-between px-6 py-3 border-t border-secondary-200">
          ...
          <div class="flex items-center space-x-2">
            <button class="px-3 py-1 text-sm border border-secondary-300 rounded hover:bg-secondary-50">Previous</button>
            <button class="px-3 py-1 text-sm bg-primary-600 text-white rounded">1</button>
            <button class="px-3 py-1 text-sm border border-secondary-300 rounded hover:bg-secondary-50">2</button>
            <button class="px-3 py-1 text-sm border border-secondary-300 rounded hover:bg-secondary-50">Next</button>
          </div>
        </div>
      </div>

      <!-- Approval Modal -->
      <div *ngIf="selectedRequest" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div class="bg-white rounded-lg shadow-xl max-w-4xl w-full mx-4 max-h-[90vh] overflow-y-auto">
          <div class="p-6">
            <div class="flex items-center justify-between mb-6">
              <h2 class="text-xl font-semibold text-secondary-900">Review Request</h2>
              <button (click)="closeApprovalModal()" class="text-secondary-400 hover:text-secondary-600">
            ...
            <div class="space-y-6">
              <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <h3 class="text-md font-medium text-secondary-900 mb-3">Request Information</h3>
                  <div class="space-y-2 text-sm">
                    <div class="flex justify-between">
                      <span class="text-secondary-600">Title:</span>
                      <span class="font-medium">{{ selectedRequest.requestTitle }}</span>
                    </div>
                    <div class="flex justify-between">
                      <span class="text-secondary-600">Type:</span>
                      <span class="font-medium">{{ getRequestTypeLabel(selectedRequest.requestType) }}</span>
                    </div>
                    <div class="flex justify-between">
                      <span class="text-secondary-600">Priority:</span>
                      <span [class]="getUrgencyClass(selectedRequest.urgency)" class="px-2 py-1 text-xs font-medium rounded-full">
                        {{ selectedRequest.urgency | titlecase }}
                      </span>
                    </div>
                    <div class="flex justify-between">
                      <span class="text-secondary-600">Risk Score:</span>
                      <span [class]="getRiskScoreClass(selectedRequest.riskScore)" class="px-2 py-1 text-xs font-medium rounded-full">
                ...
                <div>
                  <h3 class="text-md font-medium text-secondary-900 mb-3">Requester Information</h3>
                  <div class="space-y-2 text-sm">
                    <div class="flex justify-between">
                      <span class="text-secondary-600">Name:</span>
                      <span class="font-medium">{{ selectedRequest.requestedBy.name }}</span>
                    </div>
                    <div class="flex justify-between">
                      <span class="text-secondary-600">Department:</span>
                      <span class="font-medium">{{ selectedRequest.requestedBy.department }}</span>
                    </div>
                    <div class="flex justify-between">
                      <span class="text-secondary-600">Title:</span>
                      <span class="font-medium">{{ selectedRequest.requestedBy.title }}</span>
                    </div>
                    <div class="flex justify-between">
                      <span class="text-secondary-600">Manager:</span>
                      <span class="font-medium">{{ selectedRequest.requestedBy.manager || 'Not specified' }}</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div>
                <h3 class="text-md font-medium text-secondary-900 mb-3">Business Justification</h3>
                <p class="text-sm text-secondary-700 bg-secondary-50 p-3 rounded-lg">
              ...
              <div class="flex items-center justify-end space-x-3 pt-6 border-t border-secondary-200">
                ...
                <button (click)="delegateRequest(selectedRequest.id)" class="btn-secondary">Delegate</button>
                <button (click)="rejectRequest(selectedRequest.id)" class="btn-danger">Reject</button>
                <button (click)="approveRequest(selectedRequest.id)" class="btn-primary">Approve</button>
              </div>
            </div>
          </div>
        </div>
...
export class ApprovalQueueComponent implements OnInit {
  ...
  applyFilters() {
    ...
    if (this.filters.requestType) {
      filtered = filtered.filter(req => req.requestType === this.filters.requestType);
    }
    ...
    if (this.filters.queue) {
      switch (this.filters.queue) {
        case 'my_queue':
          ...
          break;
        case 'delegated':
          this.approvalService.getDelegatedApprovals().subscribe(delegated => {
            this.filteredRequests.set(delegated);
          });
          return;
        case 'escalated':
          this.approvalService.getEscalatedApprovals().subscribe(escalated => {
  ...
  processBulkAction(action: 'approve' | 'reject') {
    const actionType = action === 'approve' ? ApprovalActionType.BulkApprove : ApprovalActionType.BulkReject;
    
    this.approvalService.processBulkApprovalAction({
      action: actionType,
  ...
  delegateRequest(requestId: string) {
    ...
    this.approvalService.delegateApproval(requestId, 'delegate-001', 'Delegated for review').subscribe(() => {
      this.loadRequests();
      this.closeApprovalModal();
    });
  }
  ...
  getSlaStatusClass(request: ApprovalRequest): string {
    ...
    if (this.getTimeRemainingHours(request.deadline) < 4) return 'text-warning-600';
