    ...
    <div class="space-y-6">
      ...
      <div class="flex justify-between items-center">
        <div>
          ...
          <p class="text-secondary-600 mt-1">Monitor and review system audit events and activities</p>
        </div>
      ...
      <div *ngIf="showFilters()" class="card animate-fade-in">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <!-- Event Type Filter -->
          <div>
            <label class="block text-sm font-medium text-secondary-700 mb-1">Event Type</label>
            <select 
          ...
          <!-- Severity Filter -->
          <div>
            <label class="block text-sm font-medium text-secondary-700 mb-1">Severity</label>
            <select 
          ...
          <!-- Outcome Filter -->
          <div>
            <label class="block text-sm font-medium text-secondary-700 mb-1">Outcome</label>
            <select 
          ...
          <!-- Date From -->
          <div>
            <label class="block text-sm font-medium text-secondary-700 mb-1">From Date</label>
            <input
              type="date"
          ...
          <!-- Date To -->
          <div>
            <label class="block text-sm font-medium text-secondary-700 mb-1">To Date</label>
            <input
              type="date"
          ...
          <!-- Risk Level -->
          <div>
            <label class="block text-sm font-medium text-secondary-700 mb-1">Risk Level</label>
            <select 
          ...
          <!-- User ID -->
          <div>
            <label class="block text-sm font-medium text-secondary-700 mb-1">User ID</label>
            <input
              type="text"
            ...
            <tbody class="bg-white divide-y divide-secondary-200">
              ...
              >
                <td class="px-6 py-4 whitespace-nowrap text-sm text-secondary-900">
                  <div>
                    <div class="font-medium">{{ log.timestamp | date:'MMM d, y' }}</div>
                    <div class="text-secondary-500">{{ log.timestamp | date:'h:mm:ss a' }}</div>
                  </div>
                </td>
                <td class="px-6 py-4 whitespace-nowrap">
                  <div class="flex items-center">
                    ...
                    <div class="ml-3">
                      <div class="text-sm font-medium text-secondary-900">{{ log.userName }}</div>
                      <div class="text-sm text-secondary-500">{{ log.userRole }}</div>
                    </div>
                  </div>
                </td>
                <td class="px-6 py-4 whitespace-nowrap">
                  <div>
                    <div class="text-sm font-medium text-secondary-900">{{ log.action }}</div>
                    <div class="text-sm text-secondary-500">{{ getEventTypeLabel(log.eventType) }}</div>
                  </div>
                </td>
                <td class="px-6 py-4 whitespace-nowrap text-sm text-secondary-900">
                  {{ log.targetResource }}
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                  <div class="flex items-center">
                    <div class="flex-1">
                      <div [class]="getRiskScoreClass(log.riskScore)" class="text-sm font-medium">
                        {{ log.riskScore }}
                      </div>
        ...
        >
          <div class="px-6 py-4 border-b border-secondary-200">
            <div class="flex items-center justify-between">
              <h3 class="text-lg font-medium text-secondary-900">Audit Log Details</h3>
              <button 
                (click)="closeLogDetails()"
          ...
          <div class="px-6 py-4 max-h-[70vh] overflow-y-auto">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div class="space-y-4">
                <div>
                  <label class="block text-sm font-medium text-secondary-700">Event ID</label>
                  <p class="mt-1 text-sm text-secondary-900 font-mono">{{ selectedLog()!.id }}</p>
                </div>
                <div>
                  <label class="block text-sm font-medium text-secondary-700">Timestamp</label>
                  <p class="mt-1 text-sm text-secondary-900">{{ selectedLog()!.timestamp | date:'full' }}</p>
                </div>
                <div>
                  <label class="block text-sm font-medium text-secondary-700">User</label>
                  <p class="mt-1 text-sm text-secondary-900">{{ selectedLog()!.userName }} ({{ selectedLog()!.userRole }})</p>
                </div>
                <div>
                  <label class="block text-sm font-medium text-secondary-700">Action</label>
                  <p class="mt-1 text-sm text-secondary-900">{{ selectedLog()!.action }}</p>
                </div>
                <div>
                  <label class="block text-sm font-medium text-secondary-700">Target Resource</label>
                  <p class="mt-1 text-sm text-secondary-900">{{ selectedLog()!.targetResource }}</p>
                </div>
              </div>
              ...
              <div class="space-y-4">
                <div>
                  <label class="block text-sm font-medium text-secondary-700">Event Type</label>
                  <p class="mt-1 text-sm text-secondary-900">{{ getEventTypeLabel(selectedLog()!.eventType) }}</p>
                </div>
                <div>
                  <label class="block text-sm font-medium text-secondary-700">Category</label>
                  <p class="mt-1 text-sm text-secondary-900">{{ selectedLog()!.category }}</p>
                </div>
                <div>
                  <label class="block text-sm font-medium text-secondary-700">Severity</label>
                  <span [class]="getSeverityBadgeClass(selectedLog()!.severity)">
                    {{ selectedLog()!.severity }}
                  </span>
                </div>
                <div>
                  <label class="block text-sm font-medium text-secondary-700">Outcome</label>
                  <span [class]="getOutcomeBadgeClass(selectedLog()!.outcome)">
                    {{ selectedLog()!.outcome }}
                  </span>
                </div>
                <div>
                  <label class="block text-sm font-medium text-secondary-700">Risk Score</label>
                  <div class="flex items-center space-x-2">
                    <span [class]="getRiskScoreClass(selectedLog()!.riskScore)" class="font-medium">
            ...
            <div class="mt-6">
              ...
              <p class="mt-1 text-sm text-secondary-900">{{ selectedLog()!.details.description }}</p>
            </div>

            <!-- Technical Details -->
            <div class="mt-6" *ngIf="selectedLog()!.ipAddress || selectedLog()!.location">
              <label class="block text-sm font-medium text-secondary-700 mb-3">Technical Details</label>
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div *ngIf="selectedLog()!.ipAddress">
                  <label class="block text-sm font-medium text-secondary-600">IP Address</label>
                  <p class="mt-1 text-sm text-secondary-900 font-mono">{{ selectedLog()!.ipAddress }}</p>
                </div>
                <div *ngIf="selectedLog()!.location">
                  <label class="block text-sm font-medium text-secondary-600">Location</label>
                ...
                <div *ngIf="selectedLog()!.sessionId">
                  <label class="block text-sm font-medium text-secondary-600">Session ID</label>
                  <p class="mt-1 text-sm text-secondary-900 font-mono">{{ selectedLog()!.sessionId }}</p>
                </div>
                <div *ngIf="selectedLog()!.correlationId">
                  <label class="block text-sm font-medium text-secondary-600">Correlation ID</label>
                  <p class="mt-1 text-sm text-secondary-900 font-mono">{{ selectedLog()!.correlationId }}</p>
                </div>
              </div>
            </div>
            ...
            <!-- System Information -->
            <div class="mt-6">
              <label class="block text-sm font-medium text-secondary-700 mb-3">System Information</label>
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label class="block text-sm font-medium text-secondary-600">System</label>
                  <p class="mt-1 text-sm text-secondary-900">{{ selectedLog()!.details.systemInfo.systemName }} v{{ selectedLog()!.details.systemInfo.version }}</p>
                </div>
                <div>
                  <label class="block text-sm font-medium text-secondary-600">Environment</label>
                  <p class="mt-1 text-sm text-secondary-900">{{ selectedLog()!.details.systemInfo.environment }}</p>
                </div>
                <div>
                  <label class="block text-sm font-medium text-secondary-600">Module</label>
                  <p class="mt-1 text-sm text-secondary-900">{{ selectedLog()!.details.systemInfo.module }}</p>
                </div>
                <div *ngIf="selectedLog()!.details.systemInfo.component">
                  <label class="block text-sm font-medium text-secondary-600">Component</label>
                  <p class="mt-1 text-sm text-secondary-900">{{ selectedLog()!.details.systemInfo.component }}</p>
                </div>
              </div>
            </div>
            ...
            <div class="mt-6" *ngIf="selectedLog()!.complianceFlags.length > 0">
              <label class="block text-sm font-medium text-secondary-700 mb-2">Compliance Flags</label>
...
export class AuditLogsComponent implements OnInit {
  ...
  private applyFilters(): void {
    this.auditService.searchAuditLogs(
  ...
  hasActiveFilters(): boolean {
    ...
    return !!(f.searchQuery || f.eventType || f.category || f.severity || 
              f.outcome || f.dateFrom || f.dateTo || f.userId || f.riskLevel);
  }
  ...
  getEventTypeLabel(eventType: AuditEventType): string {
    return eventType.toString().replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase());
  }

  getSeverityBadgeClass(severity: AuditSeverity): string {
    const baseClass = 'inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium';
    switch (severity) {
      case AuditSeverity.Critical: return `${baseClass} bg-purple-100 text-purple-800`;
      case AuditSeverity.High: return `${baseClass} bg-danger-100 text-danger-800`;
      case AuditSeverity.Medium: return `${baseClass} bg-warning-100 text-warning-800`;
      case AuditSeverity.Low: return `${baseClass} bg-blue-100 text-blue-800`;
      case AuditSeverity.Informational: return `${baseClass} bg-secondary-100 text-secondary-800`;
  ...
  getOutcomeBadgeClass(outcome: AuditOutcome): string {
    const baseClass = 'inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium';
    switch (outcome) {
      case AuditOutcome.Success: return `${baseClass} bg-success-100 text-success-800`;
      case AuditOutcome.Failure: return `${baseClass} bg-danger-100 text-danger-800`;
      case AuditOutcome.Warning: return `${baseClass} bg-warning-100 text-warning-800`;
      case AuditOutcome.Error: return `${baseClass} bg-danger-100 text-danger-800`;
      case AuditOutcome.Blocked: return `${baseClass} bg-purple-100 text-purple-800`;
      default: return `${baseClass} bg-secondary-100 text-secondary-800`;
    }
  }
