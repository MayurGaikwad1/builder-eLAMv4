          ...
          <div class="flex items-center justify-between mb-4">
            <div class="text-center">
              <div
                ...
                [class]="getScoreColorClass(report.overallScore)"
              >
                {{ report.overallScore }}%
              </div>
        ...
        >
          ...
          <div class="px-6 py-4 max-h-[70vh] overflow-y-auto">
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
              ...
              <div class="lg:col-span-1 space-y-4">
                <div>
                  ...
                  <div class="mt-1 flex items-center space-x-2">
                    <span
                      class="text-2xl font-bold"
                      [class]="
                        getScoreColorClass(selectedReport()!.overallScore)
                      "
                    >
                    ...
                    <div class="flex-1 bg-secondary-200 rounded-full h-3">
                      <div
                        [class]="
                          getScoreBarClass(selectedReport()!.overallScore)
                        "
                ...
                <div>
                  ...
                  <p class="mt-1 text-sm text-secondary-900">
                    {{ selectedReport()!.framework.name }} v{{
                      selectedReport()!.framework.version
                    }}
                  </p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p class="mt-1 text-sm text-secondary-900">
                    {{
                      selectedReport()!.generatedAt | date: "MMM d, y h:mm a"
                    }}
              ...
              <div class="lg:col-span-2">
                ...
                <div class="space-y-3 max-h-96 overflow-y-auto">
                  <div
                    *ngFor="let control of selectedReport()!.controlResults"
                  ...
                  >
                    <div class="flex items-center justify-between mb-2">
                      ...
                      <span
                        [class]="getControlStatusBadgeClass(control.status)"
                      >
                        {{ control.status }}
                      </span>
                    </div>
                    <div
                      class="flex items-center justify-between text-sm text-secondary-600"
                    >
                      <span>Score: {{ control.score }}%</span>
                      <span
                        >Tested: {{ control.testedAt | date: "MMM d, y" }}</span
                      >
                    </div>
                    <p
                      *ngIf="control.comments"
                      class="text-sm text-secondary-600 mt-2"
                    >
                      {{ control.comments }}
                    </p>
                  </div>
            ...
            <div *ngIf="selectedReport()!.violations.length > 0" class="mb-6">
              ...
              <div class="space-y-3">
                <div
                  *ngFor="let violation of selectedReport()!.violations"
                  class="border border-secondary-200 rounded-lg p-3"
                >
                  <div class="flex items-start justify-between">
                    ...
                    <span
                      [class]="getViolationSeverityClass(violation.severity)"
                    >
                      {{ violation.severity }}
                    </span>
                  </div>
                  <div
                    class="flex items-center justify-between text-sm text-secondary-500 mt-2"
                  >
                    <span>{{ violation.involvedUsers[0]?.userName }}</span>
                    <span>{{
                      violation.detectedAt | date: "MMM d, h:mm a"
                    }}</span>
                  </div>
                </div>
              </div>
            ...
            <div *ngIf="selectedReport()!.findings.length > 0">
              ...
              <div class="space-y-3">
                <div
                  *ngFor="let finding of selectedReport()!.findings"
                  class="border border-secondary-200 rounded-lg p-3"
                >
                  <div class="flex items-start justify-between">
                    <div class="flex-1">
          ...
          >
            ...
            <button (click)="closeReportDetails()" class="btn-primary">
              Close
            </button>
          </div>
        </div>
      ...
      >
        ...
        >
          ...
          <div
            class="px-6 py-4 border-t border-secondary-200 flex justify-end space-x-3"
          >
            <button (click)="closeGenerateModal()" class="btn-secondary">
              Cancel
            </button>
...
export class ComplianceReportsComponent implements OnInit {
  ...
  filteredReports = computed(() => {
    ...
    if (f.framework) {
      filtered = filtered.filter(
        (report) => report.framework.name === f.framework,
      );
    }

    if (f.dateFrom) {
      filtered = filtered.filter(
        (report) => report.generatedAt >= new Date(f.dateFrom),
      );
    }

    if (f.dateTo) {
      filtered = filtered.filter(
        (report) => report.generatedAt <= new Date(f.dateTo),
      );
    }
  ...
  getControlInProgressRate(): number {
    ...
    if (metrics.controlsTotal === 0) return 0;
    return Math.round(
      (metrics.controlsInProgress / metrics.controlsTotal) * 100,
    );
  }

  getPassedControlsCount(report: ComplianceReport): number {
    return report.controlResults.filter(
