    ...
    <div class="space-y-6">
      ...
      <div class="flex items-center justify-between">
        ...
        <div class="flex items-center space-x-3">
          ...
          <button (click)="showAnalytics = true" class="btn-secondary">
            <svg
              class="w-4 h-4 mr-2"
              fill="none"
              stroke="currentColor"
        ...
        <div class="metric-card">
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-2xl font-bold text-secondary-900">
                {{ analytics()?.averageCompletionTime | number: "1.1-1" }}d
              </p>
        ...
        <div class="metric-card">
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-2xl font-bold text-secondary-900">
                {{ analytics()?.slaComplianceRate | number: "1.0-0" }}%
              </p>
              <p class="text-xs text-primary-600">
                {{ analytics()?.totalExecutions }} executions
              </p>
            </div>
            <div
        ...
        <div class="metric-card">
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-2xl font-bold text-secondary-900">
                {{ analytics()?.approvalRate | number: "1.0-0" }}%
              </p>
            ...
            >
              <div class="flex items-center justify-between">
                <div class="flex-1">
                  ...
                  <div
                    class="flex items-center space-x-4 text-xs text-secondary-500"
                  >
                    <span>{{ workflow.steps.length }} steps</span>
                    <span>v{{ workflow.version }}</span>
                    <span>{{ workflow.createdAt | date: "MMM d, y" }}</span>
                    <span *ngIf="workflow.stats"
                      >{{ workflow.stats.totalExecutions }} executions</span
                    >
                  </div>
                </div>
              ...
              >
                <div class="text-center">
                  <p class="text-lg font-semibold text-secondary-900">
                    {{ workflow.stats.averageCompletionTime | number: "1.1-1" }}
                  </p>
                ...
                <div class="text-center">
                  <p class="text-lg font-semibold text-warning-600">
                    {{ workflow.stats.slaCompliance }}%
                  </p>
      ...
      >
        ...
        >
          ...
          <div class="p-6 space-y-6">
            <!-- Performance Metrics -->
            <div>
              <h3 class="text-md font-semibold text-secondary-900 mb-4">
                ...
                <table class="min-w-full divide-y divide-secondary-200">
                  ...
                  <tbody class="bg-white divide-y divide-secondary-200">
                    <tr *ngFor="let metric of analytics()?.performanceMetrics">
                      ...
                      <td class="table-cell">
                        {{ metric.averageTime | number: "1.1-1" }}d
                      </td>
                      <td class="table-cell">
                        <span
                          [class]="getSLAComplianceClass(metric.slaCompliance)"
                        >
                      ...
                      <td class="table-cell">
                        <span
                          [class]="getApprovalRateClass(metric.approvalRate)"
                        >
                          {{ metric.approvalRate }}%
                        </span>
                      </td>
                ...
                >
                  <div>
                    ...
                    <p class="text-sm text-warning-600">
                      Average:
                      {{ bottleneck.averageTime | number: "1.1-1" }} days
                    </p>
                  </div>
                  <div class="text-right">
...
export class WorkflowManagementComponent implements OnInit {
  ...
  createWorkflow() {
    ...
    this.workflowService
      .createWorkflow(this.newWorkflow)
      .subscribe((workflow) => {
        this.loadData();
  ...
  createFromTemplate(template: WorkflowTemplate) {
    const workflowName = prompt(
      `Enter name for new workflow from template "${template.name}":`,
      template.name,
    );
    if (workflowName) {
      this.workflowService
        .createFromTemplate(template.id, { name: workflowName })
        .subscribe(() => {
          this.loadData();
  ...
  testWorkflow(workflowId: string) {
    this.workflowService.testWorkflow(workflowId, {}).subscribe((result) => {
      alert(
        `Test completed in ${result.executionTime} days with ${result.errors.length} errors`,
      );
    });
  }
  ...
  getComplexityClass(complexity: string): string {
    ...
    return (
      classes[complexity as keyof typeof classes] ||
