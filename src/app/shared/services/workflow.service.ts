...
export class WorkflowService {
  ...
  createFromTemplate(templateId: string, workflowData: Partial<Workflow>): Observable<Workflow> {
    ...
    return this.workflows$.pipe(
      map(workflows => {
        const analytics: WorkflowAnalytics = {
          ...
          totalExecutions: workflows.reduce((sum, w) => sum + (w.stats?.totalExecutions || 0), 0),
          averageCompletionTime: workflows.reduce((sum, w) => sum + (w.stats?.averageCompletionTime || 0), 0) / workflows.length,
          slaComplianceRate: workflows.reduce((sum, w) => sum + (w.stats?.slaCompliance || 0), 0) / workflows.length,
          approvalRate: workflows.reduce((sum, w) => sum + (w.stats?.approvalRate || 0), 0) / workflows.length,
          topBottlenecks: workflows.flatMap(w => w.stats?.bottlenecks || [])
  ...
  // Validation
  validateWorkflow(workflow: Workflow): Observable<{ isValid: boolean; errors: string[] }> {
    const errors: string[] = [];
